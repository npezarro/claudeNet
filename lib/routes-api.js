...
function createApiRouter(db) {
  ...
  router.post('/instances/heartbeat', (req, res) => {
    ...
    try {
      ...
      if (!instance) {
        return res.status(500).json({ error: 'Failed to retrieve instance after heartbeat' });
      }
      res.json({ ok: true, instanceId: instance.id });
    } catch (err) {
      console.error('[API] Error in heartbeat:', err);
      ...
      res.json({
        instances: instances.map(i => {
          ...
          try {
            ...
            console.error('[API] Error parsing instance metadata:', e, i.metadata);
          }
          return {
            id: i.id,
  ...
  router.patch('/instances/:id/nickname', (req, res) => {
    ...
    try {
      ...
      console.error('[API] Error setting instance nickname:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
  ...
  router.get('/thread/:threadId/settings', (req, res) => {
    ...
    console.error('[API] Error getting thread settings:', err);
      ...
      if (instanceId !== undefined) {
        ...
        db.prepare(`
          ...
          ON CONFLICT(thread_id, user_id) DO UPDATE SET
            mode = COALESCE(excluded.mode, thread_settings.mode),
            updated_at = excluded.updated_at
        `).run(req.params.threadId, req.user.id, mode || 'manual');
      }

      res.json({ ok: true });
    } catch (err) {
      console.error('[API] Error updating thread settings:', err);
      res.status(500).json({
        error: 'Internal server error',
  ...
  router.get('/thread/:threadId/poll', (req, res) => {
    ...
    try {
      ...
      res.json({
        threadId,
        mode: (settingsRow && settingsRow.mode) ? settingsRow.mode : 'manual',
      ...
      console.error('[API] Error polling thread:', err);
  ...
  router.post('/thread/:threadId/queue', (req, res) => {
    try {
      ...
      console.error('[API] Error queueing injection:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
  ...
  router.delete('/queue/:id', (req, res) => {
    try {
      ...
      console.error('[API] Error cancelling injection:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
  ...
  router.get('/inbox', (req, res) => {
    try {
      ...
      console.error('[API] Error getting inbox:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });

  // Get all messages (flattened, latest first)
  router.get('/messages', (req, res) => {
    ...
    try {
      ...
      console.error('[API] Error getting messages:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
  ...
  router.get('/thread/:threadId', (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        subject: messages.length > 0 ? messages[0].subject : "(No messages)",
        messages: messages.map(formatMessage),
      });
    } catch (err) {
      console.error('[API] Error getting thread:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
  ...
  router.get('/message/:id', (req, res) => {
    try {
      ...
      console.error('[API] Error getting message:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
  ...
  router.post('/message/:id/archive', (req, res) => {
    try {
      ...
      console.error('[API] Error archiving message:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
  ...
  router.post('/tokens/generate', (req, res) => {
    try {
      ...
      console.error('[API] Error generating token:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
  ...
  router.get('/tokens', (req, res) => {
    try {
      ...
      console.error('[API] Error listing tokens:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
  ...
  router.delete('/tokens/:id', (req, res) => {
    try {
      ...
      console.error('[API] Error revoking token:', err);
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  });
...
function formatMessage(msg) {
  let sensitivityFlags = [];
  try {
    sensitivityFlags = JSON.parse(msg.sensitivity_flags || '[]');
  } catch (e) {
    console.error('[API] Error parsing sensitivity_flags:', e, msg.sensitivity_flags);
  }
