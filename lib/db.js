...
// Seed users from environment. Set SEED_USERS as JSON array in .env, or use
// ADMIN_EMAIL/ADMIN_NAME for a single admin. Users are also auto-created on OIDC login.
function loadSeedUsers() {
  if (process.env.SEED_USERS) {
    try { return JSON.parse(process.env.SEED_USERS); } catch { /* fall through */ }
  }
  const users = [];
  if (process.env.ADMIN_EMAIL) {
    users.push({
      email: process.env.ADMIN_EMAIL,
      display_name: process.env.ADMIN_NAME || process.env.ADMIN_EMAIL.split('@')[0],
      role: 'admin',
    });
  }
  if (process.env.USER_EMAIL) {
    users.push({
      ...
      display_name: process.env.USER_NAME || process.env.USER_EMAIL.split('@')[0],
      role: 'user',
    });
  }
  return users;
...
function initDb() {
  ...
  // Migration: add target_instance_id column to thread_settings table
  const tsCols = db.pragma('table_info(thread_settings)').map(c => c.name);
  if (!tsCols.includes('target_instance_id')) {
    db.exec('ALTER TABLE thread_settings ADD COLUMN target_instance_id INTEGER REFERENCES instances(id)');
  }
  ...
  // Ensure admin role is set for configured admin (in case of existing DB)
  if (process.env.ADMIN_EMAIL) {
    db.prepare("UPDATE users SET role = 'admin' WHERE email = ?").run(process.env.ADMIN_EMAIL);
  }

  // Auto-approve bidirectional connection between seed users if exactly 2
  if (SEED_USERS.length === 2) {
    const u1 = db.prepare('SELECT id FROM users WHERE email = ?').get(SEED_USERS[0].email);
    const u2 = db.prepare('SELECT id FROM users WHERE email = ?').get(SEED_USERS[1].email);
    if (u1 && u2) {
      db.prepare(`
